"""                                 
Прошу прощения за данное обращение :D
...
# Функция  для создания папки хранения служебного файла 
def reate_a_folder():
...
# Функция для поиска пути к директории файла
def file_path(tild):
    name_file = os.path.basename(__file__)
    path_to_file = os.path.realpath(__file__).replace(name_file, tild)
    if "Temp" in path_to_file: #условие для проверки скомпилированности файла
        return(path_to_file)
    else:  
        try:
            shutil.move(path_to_file, f"{path}/favicon.ico") #перемещение картинки в рабочую директорию
...
# Функция для копирования, перевода символов, сборки и вставки текста
def fast_name():
...
# Определите функцию для выхода из окна и по совместительству для выхода из всей программы
...
def hide_window():
   ...
   icon = pystray.Icon("name", image, "Transly", menu)
   icon.run()
...
check_hotkey() #провкрка hot-key

win.mainloop() #main цикл